...
from data import tdw_dataset
from metrics import miou_metric
...
parser.add_argument('--recon_regime', default="flow",
                    type=str, help="what's being reconstructed")
parser.add_argument("--output_channels", default=3,
                    type=int, help="how many channels to output")
parser.add_argument('--model_dir', default='/om2/user/yyf/ECCV_2022/models/slot-attention/',
                    type=str, help='where to save models')
parser.add_argument(
    '--figure_dir', default='/om2/user/yyf/ECCV_2022/figures/slot-attention/')
...
def load_latest():
    ...
    sorted_checkpoints = sorted(
        checkpoints, key=lambda x: int(x.split(".")[0].split("=")[-1]))
...
flow_threshold = 0.5
train_dataloader = tdw_dataset.DataLoader(tdw_dataset.TDWDataset(tdw_dataset_dir, training=True),
                                          batch_size=opt.batch_size,
                                          shuffle=True)

model = SlotAttentionModels.SAVi(resolution, opt.num_slots, opt.num_iterations, opt.hid_dim,
    ...
    for batch in tqdm(train_dataloader):
        ...
        if (i + 1) % opt.log_every == 0:
            ...
            figure_prefix = os.path.join(
                run_dir, f"{opt.run_name}_step={i:05d}_")

            fig, axs = plt.subplots(1, 2, figsize=(12, 8))

            plot_idx = 0
            pred = recon_combined[plot_idx].detach(
